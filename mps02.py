...
class BunchDS:
    def __init__(self, ti_path, node_num, scan_frac, threshold, simul_size,
                 cond_data_path, sim_random_path=True, scan_random_path=True, bunch_size=1):
    ...
    def __str__(self):
        return 'TI_path:{}\nCond_data_path:{}\nTI_size:{}\nfacies:{}\nsimul_size:{}\nevent_node_num:{}\nmax_scan_point:{}'.format(
            self.ti_path, self.cond_data_path, self.ti_size,
            self.facies,
            self.simul_size,
    ...
    def sim2(self, it):
        ...
        # 先将它们转换称可作图矩阵
        ti_mat = self.data2mat(ti, self.ti_size)[:self.simul_size[1], :self.simul_size[0]]  # 限定模拟范围
        cond_data_mat = self.data2mat(cond_data, self.simul_size)
        ...
        iter_val = np.zeros((self.all_node, it))  # 节点值存储器
        ...
        for t in range(it):
            ...
            # 生成模拟路径
            sim_path = self.get_path(self.sim_random_path, len(simul_grid))
            # 模拟节点计数器
            sim_count = 0
            ...
            for sim_id in sim_path:
                ...
                plt.text(0, 0.2,
                         '本次训练图像共{}个点\n其中{}个点不用模拟\n每次扫描最多需扫描{}个点\n本次扫描了{}个节点\n当前模拟进度{}%'.format(
                             self.all_node, un_need_sim_point, self.max_scan_points, scan_count, sim_frac))
        ...
        #     # 更新节点值到存储器
        #     iter_val[..., t] = simul_grid[..., -1]
        #
        # # 按列取平均
        # if it == 1:
        #     final_val = iter_val
        # else:
        #     final_val = np.mean(iter_val, axis=1)
        #
        # # 作图
        # final_val_mat = self.data2mat(final_val, self.simul_size)
        # self.show_data(ti_mat, cond_data_mat, final_val_mat)
